...
#app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://pabloeluna:admindb23@pabloeluna.mysql.pythonanywhere-services.com/pabloeluna$proyecto"
...
class Calificaciones(db.Model):  # Producto hereda de db.Model
    """
    Definición de la tabla Calificacion en la base de datos.
    La clase Calificacion hereda de db.Model.
    Esta clase representa la tabla "Calificacion" en la base de datos.
    """
...
# Definición del esquema para la clase Calificacion
class CalificacionSchema(ma.Schema):
    """
    Esquema de la clase Calificacion.

    Este esquema define los campos que serán serializados/deserializados
    para la clase Calificacion.
    """
    class Meta:
        fields = ('id', 'nombre','descripcion','calificacion')

calificacion_schema = CalificacionSchema()  # Objeto para serializar/deserializar un Calificacion
calificaciones_schema = CalificacionSchema(many=True)  # Objeto para serializar/deserializar múltiples Calificaciones
...
def get_Calificaciones():
    """
    Endpoint para obtener todos las Calificaciones de la base de datos.

    Retorna un JSON con todos los registros de la tabla de Calificacion.
    """
    all_calificacion = Calificaciones.query.all()  # Obtiene todos los registros de la tabla de Calificacion
    result = calificaciones_schema.dump(all_calificacion)  # Serializa los registros en formato JSON
...
get_calificacion(id):
    # Obtiene un Calificacion específico de la base de datos
    # Retorna un JSON con la información del Calificacion correspondiente al ID proporcionado
delete_Calificacion(id):
    # Elimina un Calificacion de la base de datos
    # Retorna un JSON con el registro eliminado del Calificacion correspondiente al ID proporcionado
create_Calificacion():
    # Crea un nuevo Calificacion en la base de datos
    # Lee los datos proporcionados en formato JSON por el cliente y crea un nuevo registro de producto
    # Retorna un JSON con el nuevo Calificacion creado
update_Calificacion(id):
    # Actualiza un Calificacion existente en la base de datos
    # Lee los datos proporcionados en formato JSON por el cliente y actualiza el registro del producto con el ID especificado
    # Retorna un JSON con el Calificacion actualizado

'''
@app.route("/GetCalificaciones/<id>", methods=["GET"])
def get_calificacion(id):
    """
    Endpoint para obtener un Calificacion específico de la base de datos.
    Retorna un JSON con la información del Calificacion correspondiente al ID proporcionado.
    """
    producto = Calificaciones.query.get(id)  # Obtiene el producto correspondiente al ID recibido
    return calificacion_schema.jsonify(producto)  # Retorna el JSON del producto

@app.route("/DelCalificaciones/<id>", methods=["DELETE"])
def delete_calificacion(id):
    """
    Endpoint para eliminar un Calificacion de la base de datos.

    Elimina el Calificacion correspondiente al ID proporcionado y retorna un JSON con el registro eliminado.
    """
    calificacion = Calificaciones.query.get(id)  # Obtiene el Calificacion correspondiente al ID recibido
    db.session.delete(calificacion)  # Elimina el Calificacion de la sesión de la base de datos
    db.session.commit()  # Guarda los cambios en la base de datos
    return calificacion_schema.jsonify(calificacion)  # Retorna el JSON del Calificacion eliminado

@app.route("/AltaCalificaciones", methods=["POST"])  # Endpoint para crear un Calificacion
def create_calificacion():
    """
    Endpoint para crear un nuevo Calificacion en la base de datos.
    ...
    descripcion = request.json["descripcion"]  # Obtiene el nombre del Calificacion del JSON proporcionado
    calificacion = request.json["calificacion"]  # Obtiene el precio del Calificacion del JSON proporcionado
    new_calificacion = Calificaciones(nombre,descripcion,calificacion)  # Crea un nuevo objeto Producto con los datos proporcionados
    db.session.add(new_calificacion)  # Agrega el nuevo Calificacion a la sesión de la base de datos
    db.session.commit()  # Guarda los cambios en la base de datos
    return calificacion_schema.jsonify({'prop':'new_calificacion'})  # Retorna el JSON del nuevo Calificacion creado

@app.route("/UpdCalificaciones/<id>", methods=["PUT"])  # Endpoint para actualizar un Calificacion
def update_calificacion(id):
    """
    Endpoint para actualizar un Calificacion existente en la base de datos.

    Lee los datos proporcionados en formato JSON por el cliente y actualiza el registro del Calificacion con el ID especificado.
    Retorna un JSON con el Calificacion actualizado.
    """
    calificacion = Calificaciones.query.get(id)  # Obtiene el Calificacion existente con el ID especificado

    # Actualiza los atributos del Calificacion con los datos proporcionados en el JSON
    calificacion.nombre = request.json['nombre']
    calificacion.descripcion = request.json['descripcion']
    calificacion.calificacion = request.json['calificacion']
    

    db.session.commit()  # Guarda los cambios en la base de datos
    return calificacion_schema.jsonify(calificacion)  # Retorna el JSON del Calificacion actualizado
